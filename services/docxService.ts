...
export const generateDocx = async (data: DidacticSequence, input: SequenceInput) => {
  ...
  const logoUrl = input.logo_url || '/logo_placeholder.png'; 
  try {
            ...
            rows: [
              new TableRow({
                children: [
                  ...
                  createCell([
                    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: input.nombre_institucion || "INSTITUCIÓN EDUCATIVA", bold: true, size: 20 })] }),
          ...
          new Table({
            ...
            rows: [
              new TableRow({
                children: [
                  createCell([
                    new Paragraph({ children: [new TextRun({ text: "DBA: ", bold: true }), new TextRun({ text: data.dba_detalle?.numero || (data.dba_utilizado?.includes("DBA") ? data.dba_utilizado : "") || input.dba || "Consultar referentes MEN" })] }),
                    new Paragraph({ text: data.dba_detalle?.enunciado || (!data.dba_detalle?.numero ? data.dba_utilizado : "") || "" })
                  ], { width: 50 }),
                  createCell([
  ...
  const safeInstName = (input.nombre_institucion || "EduPlaneacion").replace(/\s+/g, '_');
  const fileName = `Planeacion_${safeInstName}_${data.titulo_secuencia.replace(/\s+/g, '_')}.docx`;
