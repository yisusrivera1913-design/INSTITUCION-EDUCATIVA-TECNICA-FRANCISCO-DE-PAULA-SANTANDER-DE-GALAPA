    ...
    <div className="animate-fade-in-up pb-10">
      ...
      <div className="bg-white sticky top-20 z-40 p-5 rounded-2xl shadow-md border border-slate-200 mb-8 no-print flex justify-between items-center transition-all">
        <div className="flex items-center gap-4 pl-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800 tracking-tight">Formato Nacional de Planeación Clave v2.0</h2>
            <p className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">
              {input.nombre_institucion || "Sistema de Gestión Académica"}
            </p>
          </div>
        </div>
      ...
      <div id="preview-container" className="bg-white mx-auto max-w-[21.5cm] min-h-[29.7cm] p-[1.5cm] text-black border border-slate-300 shadow-xl print:shadow-none print:border-none print:p-0 print:w-full font-serif relative">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] opacity-[0.02] pointer-events-none z-0 flex items-center justify-center">
          {input.logo_url ? (
            <img src={input.logo_url} alt="" className="w-full h-full object-contain grayscale" />
          ) : (
            <Sparkles className="w-full h-full text-slate-200" />
          )}
        </div>
          ...
          <div className="border-[1px] border-black w-full mb-8 text-black bg-white">
            ...
            <div className="flex border-b border-black h-[100px]">
              <div className="w-[120px] p-2 flex items-center justify-center border-r border-black">
                {input.logo_url ? (
                  <img src={input.logo_url} alt="Escudo" className="w-[90px] h-auto object-contain" />
                ) : (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-2 text-center">
                <h1 className="text-[14px] font-black uppercase leading-tight px-4">
                  {input.nombre_institucion || "INSTITUCIÓN EDUCATIVA NACIONAL"}
                </h1>
              ...
              <div>
                ...
                <div className="bg-white p-2 border border-black italic">
                  <p className="font-black">
                    {editableData.dba_detalle?.numero || (editableData.dba_utilizado?.includes("DBA") ? editableData.dba_utilizado : "") || input.dba || "Referente Normativo MEN"}
                  </p>
                  <p>{editableData.dba_detalle?.enunciado || (!editableData.dba_detalle?.numero ? editableData.dba_utilizado : "")}</p>
                </div>
              </div>
              <div>
          ...
          {editableData.taller_imprimible && (
            <div className="break-before-page border-t-2 border-black pt-8 mt-12">
              <div className="flex justify-between items-center mb-6">
                <div className="w-[100px] h-[100px] mb-2 flex items-center justify-center bg-blue-600 rounded-2xl shadow-md">
                   <Sparkles className="text-white w-12 h-12" />
                </div>
                <div className="text-center">
                  <h1 className="text-xl font-black text-slate-900 tracking-tighter uppercase leading-none">EduPlaneación AI</h1>
                  <p className="text-[9px] font-black text-blue-600 uppercase tracking-[2px] mt-1">Soberanía Pedagógica</p>
                </div>
                <div className="text-right border-l border-black pl-4">
                  <p className="text-[12px] font-black uppercase">{input.codigo_formato || 'F-GEN'}</p>
                  <p className="text-[8px] font-bold text-slate-400 uppercase">EduPlaneación AI Platform</p>
                </div>
              </div>
